...
| **159 條護欄（絕對不可單點修改）** | `CLAUDE_call-monitor-guardrails.md`（原 §7，2026-09-04 起獨立成檔） |
| 這段程式碼為什麼長這樣（42 輪修復年表；近期輪次在 §8，第一至三十五輪在 `CLAUDE_call-monitor-history.md`） | §8 |
...
14. **App 畫面不得出現 Flutter 的 RenderFlex 溢位警示（黃黑斜紋條）** — 任何畫面在任何裝置尺寸下都不得出現 `RIGHT/BOTTOM OVERFLOWED BY N PIXELS`；這是使用者看得見的缺陷，等同 UI bug。常見成因與修法：`Row` 內的 `Text` 未包 `Expanded`／`Flexible` 且內容長度不可控（使用者自訂裝置名稱、長輩姓名、後端回傳訊息）→ 包 `Flexible` 並加 `overflow: TextOverflow.ellipsis`；固定寬度元件在窄螢幕溢位 → 改用相對寬度或 `Wrap`。**新增或修改任何含 `Row` + 動態字串的 UI 前，必須確認該 `Text` 可收縮。**⚠️ 本規則不豁免 UI 地圖與護欄的既有要求；純樣式修正若動到通話／監控相關畫面，仍須先讀 `CLAUDE_call-monitor.md`。現況（2026-09-01 掃描）：全 App 有 213 處 `Row` 含 `Text` 但無 `Expanded`／`Flexible`，其中 94 處是長度不可控的動態字串；**不要為此一次全部修改**——多數是短標籤不會溢位，盲改必然造成回歸，正確做法是實機發現一處、精準修一處，並在該處加註解說明為什麼需要可收縮。**每輪例行檢查義務（2026-09-04 起，第四十一輪新增）**：每一輪更新完成前（只要 git 狀態有改動、輪次編號要遞增），必須檢查**雙端**的標籤主介面是否出現溢位——**家屬端不可省略**。要檢查的畫面固定為以下 8 個：長輩端 5 個（首頁、電話、社群、聊天、我的）、家屬端 3 個（首頁、互動、資料）。檢查方式：至少對這 8 個畫面做「同列有 ≥18pt 標題且同列還有其他元素（徽章／箭頭／按鈕）時，標題是否可收縮」的靜態核對；有實機時以實機為準。**判準是「字級 × 同列元素數」，不是「字串是不是動態」**——第三十九輪就是用錯判準漏掉兩處（`'家庭生活時光牆'` 22pt、`'AI 照護共創助理'` 18pt），第四十輪才補上。這條例行義務與前段「不要一次全部盲改 213 處」並存不衝突：例行檢查只鎖定這 8 個主介面，不是全 App 逐一排查。

### 3.2 通話與監控

**完整規則見 [`CLAUDE_call-monitor-guardrails.md`](CLAUDE_call-monitor-guardrails.md)（159 條護欄）。**
...
通話與監控子系統的完整修復年表（2026-06-05 起，已累積 42 輪）已逐條核對，內容全數存在於
