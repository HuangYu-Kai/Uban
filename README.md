...
#### 🌟 現行核心功能
- **無障礙極簡語音介面**：大字體、全語音優先對話介面，防誤觸極簡五分頁導覽（首頁、電話、社群、聊天、我的）。
- **首頁一屏到底（不需滾動）** ⭐ 第四十六輪：依 `ElderScale`「每屏選項少」原則，首頁只保留三塊——
  「今天」卡（國曆／星期／農曆／節氣／天氣／農民曆入口）、「下一包藥」、「今日頭條」。
  新聞卡依實際可用高度伸縮：長輩手機（可用約 510px）維持縮圖橫列，平板或桌機視窗則長成滿版大圖，
  兩種尺寸都不需要捲動。移除與底部導覽列「電話」重複的撥號大按鈕。
- **「我的」分頁即小豬之家** ⭐ 第四十六輪：採 Pokémon GO 詳情頁的版面結構（滿版主視覺 ＋ 大隻角色
  ＋ 白色圓角資訊卡壓在主視覺上），但配色仍守 `ART_STYLE_GUIDE_PIGLET.md`——背景沿用手繪花園圖加
  高斯模糊與淡化凸顯小豬，不使用霓虹或高飽和數位漸層。資訊卡顯示階段稱號、成長進度、體重／階段／
  活力三欄與「餵小豬」主按鈕（原本藏在背景圖的隱形熱區，長輩無從發現）。
- **主動關懷留下痕跡** ⭐ 第四十七輪：後端每分鐘評估的主動關懷訊息，除了 TTS 朗讀外，
  首頁會顯示 `HeartbeatOverlay` 對話框、留存最近 30 則（`CareMessageStore`），並接進聊天分頁成為
  小嘎的一則訊息。先前只有聲音、畫面不留痕，重聽或靜音的長輩會完全錯過且無法回溯。
...
- **用藥打卡三路一致** ⭐ 第四十七輪：清單點擊、提醒彈窗、對小嘎說「我吃過藥了」三條路徑，
  最終都會收斂到同一份本機當日完成清單（`completed_tasks_<yyyy-MM-dd>`）。
  `reminder-sync` 信令新增 `reminderId` 欄位，長輩端據此更新本機狀態。
  ⚠️ 語音打卡在藥名對不上、且當天有多筆用藥提醒時，會據實告知長輩「畫面上的打卡要自己點」，
  不會代為認定——寧可讓長輩多點一下，也不要打錯別包藥的卡。
...
**2026-09-22 續做**
- 🎙️ **語音助理全域入口**：助理原本只有兩個呼叫點，兩個都活在 `ElderHomeScreen` 的
  `Stack` 裡，只覆蓋 5 個分頁——通話房、監控、新聞播放器、配對頁等 `Navigator.push`
  出去的畫面都叫不出來。新增可拖曳、放開吸邊、閒置 5 秒收成半透明小圓點的浮動麥克風鈕
  （`widgets/global_assistant_button.dart`），掛在 `MaterialApp.builder` 這一層，位置與
  收邊狀態存 SharedPreferences。**助理邏輯沒有複製第二份**：長輩端首頁把既有的
  `_triggerGoogleAssistantOverlay` 登記為全域啟動器，浮動鈕只負責呼叫。
  通話房、監控、來電響鈴畫面與助理面板本身都會讓它隱藏（護欄 **G199**），不會擋到
  接聽／掛斷鍵。
- 🎤 **語音喚醒預設值回到「關閉」**：首頁載入設定時每次都把喚醒詞旗標強制寫回 `true`，
  長輩在設定頁關掉、下次進首頁又被打開（麥克風無限開開關關）。首頁改為只讀不寫，
  唯一寫入點回到設定頁——這是護欄 **G59** 的原文要求。
- 🧹 **離線不再吃掉社群貼文**：`CommunityApi.getCommunityPosts` 把連線失敗吞掉回空陣列，
  讓「離線」與「後端說一則都沒有」變得無法分辨，離線時會拿空清單覆蓋本機快取。
  改為 **null＝呼叫失敗、空清單＝真的沒貼文**。
