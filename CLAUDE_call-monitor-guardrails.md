...
> 🗂️ **這是什麼**：通話與監控子系統全部 205 條護欄的**索引檔**。護欄正文（原
> `CLAUDE_call-monitor.md` §7 的完整內容）2026-09-04 第四十二輪從主文件獨立成本檔，之後隨
> 每輪真機故障持續累積；2026-09-24 第五十三輪本檔已達 200,175 bytes，再依前端／後端拆成兩卷：
> `CLAUDE_call-monitor-guardrails-frontend.md`（§7.1，111 條）與
> `CLAUDE_call-monitor-guardrails-backend.md`（§7.2，94 條）。本檔（索引）只保留表頭、
...
>
> **收錄範圍**：全部 205 條護欄（G1–G205）分存兩卷——§7.1 前端護欄 111 條在
> `CLAUDE_call-monitor-guardrails-frontend.md`；§7.2 後端護欄 94 條在
...
> 🗂️ **§7.1（前端護欄，111 條）與 §7.2（後端護欄，94 條）已搬到**
