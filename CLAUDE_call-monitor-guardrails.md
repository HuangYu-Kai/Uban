...
# CLAUDE_call-monitor-guardrails.md — 通話與監控子系統 護欄清單

> 🗂️ **這是什麼**：`CLAUDE_call-monitor.md` §7 的護欄本文，2026-09-04 第四十二輪從主文件獨立
> 出來，內容逐字搬移、未經改寫、未重新編號。
>
> **為什麼獨立**：主文件曾成長到 261,793 bytes，逼近工具單次讀取上限（262,144 bytes），使
> 「動手前必須完整讀過本文件」這條鐵律在技術上無法遵守。§7 護欄本文獨佔主文件約 55%
> （145,620 bytes），是逼近上限的主要成因——護欄只會隨每次真機故障持續累積、不像 §8 年表
> 能把舊條目遷往 `CLAUDE_call-monitor-history.md`，鐵律 #12 早已預告「下一刀要切的是 §7」，
> 本次即是執行。
>
> ⚠️ **這份是動手前必讀的一部分，不是查證用的史料**——與純歷史存檔的
...
>
> **收錄範圍**：全部 159 條護欄（G1–G159），即原文件 §7 的完整內容：
> §7.1 前端護欄／§7.2 後端護欄／§7.3 已知的文件錯誤（以程式碼為準）／
> §7.4 已知且刻意保留的安全缺口。
>
> **本文件中所有「§7」「G數字」的引用，一律指向本檔**；`CLAUDE_call-monitor.md`、
> `CLAUDE_call-monitor-history.md`、`CLAUDE_call-monitor-ui-map.md` 與三份 `CLAUDE.md` 中出現
> 的既有「見 §7」「見 G12」等寫法，同樣一律指向本檔，不逐處改寫。

---

## 7. 護欄（合併後的唯一權威清單）

> 目前共 **159 條**（G1–G159）：G1–G36 合併自 `CLAUDE.md`（13 條）與 `Uban/CLAUDE.md`（26 條）並去重、
...
係不該被誤擋。
🚫 **禁止**擋 `/dev/ensure-yuxuan-demo`／`/dev/ensure-gawa-demo` 等開發測試端點——它們
的 `family_id` 是寫死常數，擋了會讓既有測試路徑無預警壞掉，這是刻意的豁免。
