...
class DesktopPetState extends State<DesktopPet> {
  ...
  void didChangeDependencies() {
    ...
    precacheImage(const AssetImage('assets/images/pig_2d_picked_v5.png'), context);
  }
  ...
  void _startStateMachine() {
    ...
    _stateTimer = Timer(duration, () {
      if (!mounted || _currentState == PetState.happy || _currentState == PetState.pickedUp) return;
      
      final r = _random.nextDouble();
      
      // 根據心情調整狀態機率
      if (_currentMood == PetMood.lazy || _currentMood == PetMood.tired) {
        if (r < 0.2) _changeState(PetState.walking);
        else if (r < 0.7) _changeState(PetState.sleeping);
        else _changeState(PetState.idle);
      } else if (_currentMood == PetMood.energetic) {
        if (r < 0.6) _changeState(PetState.walking);
        else if (r < 0.8) _changeState(PetState.idle);
        else _changeState(PetState.sleeping);
      } else {
        if (r < 0.4) _changeState(PetState.walking);
        else if (r < 0.6) _changeState(PetState.sleeping);
        else _changeState(PetState.idle);
      }
      _startStateMachine();
    });
  ...
  void _changeState(PetState newState) {
    ...
    setState(() {
      ...
      if (newState == PetState.walking) {
        ...
        double distance = sqrt(pow(targetX - _positionX, 2) + pow(targetY - _positionY, 2));
        ...
        _walkDuration = Duration(milliseconds: (distance / speed * 1000).toInt());
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedPositioned(
      duration: (_currentState == PetState.walking) 
          ? _walkDuration 
          : (_currentState == PetState.pickedUp ? Duration.zero : const Duration(milliseconds: 600)),
      ...
      child: GestureDetector(
        ...
        onLongPressMoveUpdate: (details) {
          setState(() {
            ...
            _positionX = (_dragStartX - details.offsetFromOrigin.dx).clamp(0.0, 280.0);
            // 因為是 bottom，手指往上走 (dy < 0)，bottom 應該要增加
            _positionY = (_dragStartY - details.offsetFromOrigin.dy).clamp(0.0, 500.0);
          });
        },
        ...
        child: Column(
          ...
          children: [
            if (_currentMood == PetMood.energetic)
              _buildMoodBadge("活力百倍 ✨"),
            if (_currentMood == PetMood.lazy)
              _buildMoodBadge("想去散步 👣"),
            if (_currentMood == PetMood.tired)
              _buildMoodBadge("休息時間 💤"),
            ...
            Transform(
              ...
              transform: Matrix4.identity()
                ..scale(_getLevelScale(_level) * (_currentState == PetState.pickedUp ? 1.2 : 1.0))
              ...
              child: Stack(
                ...
                children: [
                  ...
                  if (_currentMood == PetMood.energetic)
                    ...
                    ).animate(onPlay: (c) => c.repeat()).scaleXY(begin: 0.8, end: 1.2, duration: 1.seconds, curve: Curves.easeInOut),
              ...
              target: (_currentState == PetState.idle || _currentState == PetState.happy) ? 1 : 0,
            ...
            .scaleXY(begin: 1.0, end: 1.1, duration: 250.ms, curve: Curves.elasticOut),
          ],
        ),
      ),
    );
