...
> 📌 **搬移門檻提示**：本文件中出現的「第 N 輪」，**N ≤ 33** 者其年表條目已遷至
> `CLAUDE_call-monitor-history.md`；**N ≥ 34** 仍在本檔 §8。此門檻會隨每輪搬移而持續調高，
> 調整時只需要更新這兩處（本節與 §8 開頭）的數字。

> 目前共 **134 條**（G1–G134）：G1–G36 合併自 `CLAUDE.md`（13 條）與 `Uban/CLAUDE.md`（26 條）並去重、
